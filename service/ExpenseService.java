...
public class ExpenseService {
    ...
    // private void divideExpenseWithExactAmounts(Float amount, String payingUser,
    // Map<String, Float> splitMap) {
    // User payingUserData = splitdata.getUserMap().get(payingUser);
    // boolean isFirstUser = true;
    // for (String userId : splitMap.keySet()) {
    // User currentUser = splitdata.getUserMap().get(userId);
    // Float share = getRoundedOffShare(splitMap.get(userId), isFirstUser);
    // isFirstUser = false;
    // if (userId != payingUser) {
    // // update data in current user
    // Float balanceOfPayingUserInCurrentUser =
    // currentUser.getBalances().get(payingUser);
    // balanceOfPayingUserInCurrentUser -= share;
    // currentUser.getBalances().put(payingUser, balanceOfPayingUserInCurrentUser);
    // // update data in paying user
    // Float balanceOfCurrentUserInPayingUser =
    // payingUserData.getBalances().get(userId);
    // balanceOfCurrentUserInPayingUser += share;
    // payingUserData.getBalances().put(userId, balanceOfCurrentUserInPayingUser);
    // }
    // }
    // System.out.println("Expense divided with Exact Amount");
    // }

    // private void divideExpenseWithPercentage(Float amount, String payingUser,
    // Map<String, Float> splitMap) {
    // User payingUserData = splitdata.getUserMap().get(payingUser);
    // boolean isFirstUser = true;
    // for (String userId : splitMap.keySet()) {
    // User currentUser = splitdata.getUserMap().get(userId);
    // Float share = getRoundedOffShare(((splitMap.get(userId) * amount) / 100),
    // isFirstUser);
    // isFirstUser = false;
    // if (userId != payingUser) {
    // // update data in current user
    ...
    // }
    // }
    // System.out.println("Expense divided with Percentage");
    // }
